...
def load_data():
    ...
    temps_df = pd.read_csv(data_path, parse_dates=["Date"])  # TODO: Ex 3.1: Load the dataset using Pandas, use the data_path variable and set the index column to "show_id"
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    ax.set_title("Temperatures Over Time for Selected Cities")
    ax.set_xlabel("Date")
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df["City"] == city]
        city_df_period = city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)]
        ax.hist(city_df_period["AvgTemperatureCelsius"], bins=20, alpha=0.5, label=city)

    ax.set_title("Temperature Histogram for Selected Cities")
